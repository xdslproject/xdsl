// RUN: xdsl-opt %s -p convert-linalg-to-memref-stream | filecheck %s
...
// CHECK-NEXT:    %0 = "memref_stream.stride_pattern"() {"ub" = [#builtin.int<8>, #builtin.int<16>], "strides" = [#builtin.int<16>, #builtin.int<1>], "dm" = #builtin.int<31>} : () -> !memref_stream.stride_pattern_type<2>
// CHECK-NEXT:    %1 = arith.constant 128 : index
// CHECK-NEXT:    "memref_stream.generic"(%1, %X, %Y, %Z, %0) <{"operandSegmentSizes" = array<i32: 1, 2, 1, 1>}> ({
// CHECK-NEXT:    ^0(%x : f64, %y : f64):
// CHECK-NEXT:      %r0 = arith.addf %x, %y : f64
// CHECK-NEXT:      "memref_stream.yield"(%r0) : (f64) -> ()
// CHECK-NEXT:    }) : (index, memref<8x16xf64>, memref<8x16xf64>, memref<8x16xf64>, !memref_stream.stride_pattern_type<2>) -> ()
// CHECK-NEXT:  }
