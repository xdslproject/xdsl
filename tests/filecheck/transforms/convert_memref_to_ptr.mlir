...
// CHECK-NEXT:  %bytes_per_element_7 = ptr_xdsl.type_offset i32 : index
// CHECK-NEXT:  %scaled_pointer_offset_7 = arith.muli %c5, %bytes_per_element_7 : index
// CHECK-NEXT:  %offset_pointer_7 = ptr_xdsl.ptradd %arr_3, %scaled_pointer_offset_7 : (!ptr_xdsl.ptr, index) -> !ptr_xdsl.ptr
// CHECK-NEXT:  %subview = ptr_xdsl.from_ptr %offset_pointer_7 : !ptr_xdsl.ptr -> memref<5xi32>

%size, %dyn = "test.op"() : () -> (index, memref<?xi32>)
%dynsubview = memref.subview %dyn[%idx][%size][1] : memref<?xi32> to memref<?xi32>

// CHECK-NEXT:  %size, %dyn = "test.op"() : () -> (index, memref<?xi32>)
// CHECK-NEXT:  %dyn_1 = ptr_xdsl.to_ptr %dyn : memref<?xi32> -> !ptr_xdsl.ptr
// CHECK-NEXT:  %bytes_per_element_8 = ptr_xdsl.type_offset i32 : index
// CHECK-NEXT:  %scaled_pointer_offset_8 = arith.muli %idx, %bytes_per_element_8 : index
// CHECK-NEXT:  %offset_pointer_8 = ptr_xdsl.ptradd %dyn_1, %scaled_pointer_offset_8 : (!ptr_xdsl.ptr, index) -> !ptr_xdsl.ptr
// CHECK-NEXT:  %dynsubview = ptr_xdsl.from_ptr %offset_pointer_8 : !ptr_xdsl.ptr -> memref<?xi32>
// -----
