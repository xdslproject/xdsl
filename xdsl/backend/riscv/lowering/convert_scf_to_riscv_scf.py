...
class ScfForLowering(RewritePattern):
    ...
    def match_and_rewrite(self, op: scf.For, rewriter: PatternRewriter) -> None:
        ...
        mv_op_pairs = [
            move_ops_for_value(arg, rd=register_type_for_type(arg.type).unallocated())
            for arg in args
        ]
        rewriter.insert_op_before_matched_op([mv_op for mv_op, _ in mv_op_pairs])
        cast_matched_op_results(rewriter)
        rewriter.replace_matched_op(
            riscv_scf.ForOp(
                lb, ub, step, [value for _, value in mv_op_pairs], new_region
            )
        )
