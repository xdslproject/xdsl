...
builtin.module {
  ...
  // CHECK: "f10"() {"map" = affine_map<(d0, d1) -> (((d1 + (d0 * -1)) + ((((((d0 + (d1 * -1)) + 1) * 2) + d1) + -1) + 0)), (((((d1 + d1) + 1) + d1) + d1) + 1))>} : () -> ()
  "f10"() { map = affine_map<(i, j) -> ((j - i) + 2*(i - j + 1) + j - 1 + 0, j + j + 1 + j + j + 1)> } : () -> ()

  // CHECK: "f11"() {"map" = affine_map<(d0, d1) -> ((((d0 + 4) * -1) + 6), d1)>} : () -> ()
  "f11"() { map = affine_map<(i, j) -> (3+3-2*2+i, j)>} : () -> ()

  // CHECK: "f12"() {"map" = affine_map<(d0, d1) -> (((d0 * 1) + 3), ((d1 * 4) + 2))>} : () -> ()
  ...
  // CHECK: "f14"() {"map" = affine_map<(d0, d1, d2) -> ((d0 * 0), (d1 * 1), ((d0 * 128) floordiv 64), ((d1 * 0) floordiv 64))>} : () -> ()
  "f14"() { map = affine_map<(i, j, k) -> (i*0, 1*j, i * 128 floordiv 64, j * 0 floordiv 64)>} : () -> ()

  // CHECK: "f16"() {"map" = affine_map<(d0, d1) -> (8, 4, 1, 3, 2, 4)>} : () -> ()
  "f16"() { map = affine_map<(i, j) -> (5+3, 2*2, 8-7, 100 floordiv 32, 5 mod 3, 10 ceildiv 3)>} : () -> ()

  // CHECK: "f17"() {"map" = affine_map<(d0, d1) -> (4, 11, 512, 15)>} : () -> ()
  "f17"() { map = affine_map<(i, j) -> (5 mod 3 + 2, 5*3 - 4, 128 * (500 ceildiv 128), 40 floordiv 7 * 3)>} : () -> ()
}
