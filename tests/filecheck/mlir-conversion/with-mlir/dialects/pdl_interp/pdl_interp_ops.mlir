...
// CHECK-NEXT:       %0 = "pdl_interp.get_result"(%arg5) <{index = 0 : i32}> : (!pdl.operation) -> !pdl.value
// CHECK-NEXT:       "pdl_interp.is_not_null"(%0) [^1, ^2] : (!pdl.value) -> ()
// CHECK-NEXT:     ^2:
// CHECK-NEXT:       "pdl_interp.finalize"() : () -> ()
// CHECK-NEXT:     ^1:
// CHECK-NEXT:       %1 = "pdl_interp.get_operand"(%arg5) <{index = 0 : i32}> : (!pdl.operation) -> !pdl.value
// CHECK-NEXT:       %2 = "pdl_interp.get_defining_op"(%1) : (!pdl.value) -> !pdl.operation
// CHECK-NEXT:       "pdl_interp.is_not_null"(%2) [^3, ^2] : (!pdl.operation) -> ()
// CHECK-NEXT:     ^3:
// CHECK-NEXT:       "pdl_interp.check_operation_name"(%arg5) [^4, ^2] <{name = "arith.subi"}> : (!pdl.operation) -> ()
// CHECK-NEXT:     ^4:
// CHECK-NEXT:       "pdl_interp.check_operand_count"(%arg5) [^5, ^2] <{count = 2 : i32}> : (!pdl.operation) -> ()
// CHECK-NEXT:     ^5:
// CHECK-NEXT:       "pdl_interp.check_result_count"(%arg5) [^6, ^2] <{count = 1 : i32}> : (!pdl.operation) -> ()
// CHECK-NEXT:     ^6:
// CHECK-NEXT:       "pdl_interp.is_not_null"(%1) [^7, ^2] : (!pdl.value) -> ()
// CHECK-NEXT:     ^7:
// CHECK-NEXT:       %3 = "pdl_interp.get_operand"(%arg5) <{index = 1 : i32}> : (!pdl.operation) -> !pdl.value
// CHECK-NEXT:       "pdl_interp.is_not_null"(%3) [^8, ^2] : (!pdl.value) -> ()
// CHECK-NEXT:     ^8:
// CHECK-NEXT:       "pdl_interp.check_operation_name"(%2) [^9, ^2] <{name = "arith.addi"}> : (!pdl.operation) -> ()
// CHECK-NEXT:     ^9:
// CHECK-NEXT:       "pdl_interp.check_operand_count"(%2) [^10, ^2] <{count = 2 : i32}> : (!pdl.operation) -> ()
// CHECK-NEXT:     ^10:
// CHECK-NEXT:       "pdl_interp.check_result_count"(%2) [^11, ^2] <{count = 1 : i32}> : (!pdl.operation) -> ()
// CHECK-NEXT:     ^11:
// CHECK-NEXT:       %4 = "pdl_interp.get_operand"(%2) <{index = 0 : i32}> : (!pdl.operation) -> !pdl.value
// CHECK-NEXT:       "pdl_interp.is_not_null"(%4) [^12, ^2] : (!pdl.value) -> ()
// CHECK-NEXT:     ^12:
// CHECK-NEXT:       %5 = "pdl_interp.get_operand"(%2) <{index = 1 : i32}> : (!pdl.operation) -> !pdl.value
// CHECK-NEXT:       "pdl_interp.is_not_null"(%5) [^13, ^2] : (!pdl.value) -> ()
// CHECK-NEXT:     ^13:
// CHECK-NEXT:       %6 = "pdl_interp.get_result"(%2) <{index = 0 : i32}> : (!pdl.operation) -> !pdl.value
// CHECK-NEXT:       "pdl_interp.is_not_null"(%6) [^14, ^2] : (!pdl.value) -> ()
// CHECK-NEXT:     ^14:
// CHECK-NEXT:       "pdl_interp.are_equal"(%6, %1) [^15, ^2] : (!pdl.value, !pdl.value) -> ()
// CHECK-NEXT:     ^15:
// CHECK-NEXT:       %7 = "pdl_interp.get_value_type"(%6) : (!pdl.value) -> !pdl.type
// CHECK-NEXT:       %8 = "pdl_interp.get_value_type"(%0) : (!pdl.value) -> !pdl.type
// CHECK-NEXT:       "pdl_interp.are_equal"(%7, %8) [^16, ^2] : (!pdl.type, !pdl.type) -> ()
// CHECK-NEXT:     ^16:
// CHECK-NEXT:       "pdl_interp.record_match"(%5, %3, %7, %4, %arg5, %2, %arg5) [^2] <{benefit = 1 : i16, generatedOps = ["arith.subi", "arith.addi"], operandSegmentSizes = array<i32: 5, 2>, rewriter = @rewriters::@pdl_generated_rewriter, rootKind = "arith.subi"}> : (!pdl.value, !pdl.value, !pdl.type, !pdl.value, !pdl.operation, !pdl.operation, !pdl.operation) -> ()
// CHECK-NEXT:     }) : () -> ()
// CHECK-NEXT:     "builtin.module"() <{sym_name = "rewriters"}> ({
...
// CHECK-NEXT:         %1 = "pdl_interp.get_result"(%0) <{index = 0 : i32}> : (!pdl.operation) -> !pdl.value
...
// CHECK-NEXT:         %3 = "pdl_interp.get_result"(%2) <{index = 0 : i32}> : (!pdl.operation) -> !pdl.value
// CHECK-NEXT:         %4 = "pdl_interp.get_results"(%2) : (!pdl.operation) -> !pdl.range<value>
// CHECK-NEXT:         "pdl_interp.replace"(%arg4, %4) : (!pdl.operation, !pdl.range<value>) -> ()
// CHECK-NEXT:         "pdl_interp.finalize"() : () -> ()
// CHECK-NEXT:       }) : () -> ()
// CHECK-NEXT:     }) : () -> ()
// CHECK-NEXT:   }) : () -> ()
