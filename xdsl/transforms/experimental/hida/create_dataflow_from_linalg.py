...
from xdsl.dialects.experimental.utils import dispatch_block, fuse_ops_into_task, is_element_wise_generic_op
from xdsl.dialects import linalg, arith, tensor, func, builtin
...
class OutlineConvolution(RewritePattern):
    @op_type_rewrite_pattern
    def match_and_rewrite(self, op : linalg.ConvOpsBase | linalg.PoolingOpsBase, rewriter : PatternRewriter):
        if op.get_parent_of_type(TaskOp):
            return
...
class ForwardFuseLinalgGeneric(RewritePattern):
    ...
    def match_and_rewrite(self, op : linalg.Generic, rewriter : PatternRewriter, /):
        ...
        if len(op.outputs) == 1 and len(op.body.block.ops) == 1:
            ...
            if isinstance(output.owner, arith.Constant): # TODO: or isinstance(output.owner, tosa.Constant)
                matched = True
...
class DispatchBlocks(RewritePattern):
    ...
    def match_and_rewrite(self, op : func.FuncOp, rewriter : PatternRewriter, /):
        ...
        empty_tensors = [empty for empty in filter(lambda x: isinstance(x, tensor.EmptyOp), op.walk())]
...
class CreateDataflowFromLinalg(ModulePass):
    ...
    def apply(self, ctx: MLContext, op: builtin.ModuleOp) -> None:
        ...
        fuse_pass_2 = PatternRewriteWalker(
            GreedyRewritePatternApplier(
                [OutlineLinalgOp()] + fuse_pass_calls
            ),
            walk_reverse=True
        )
