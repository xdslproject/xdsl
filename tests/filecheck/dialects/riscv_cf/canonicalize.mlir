...
// CHECK-NEXT:    riscv_cf.branch ^bb1(%0 : !riscv.reg) attributes {comment = "Constant folded riscv_cf.bge"}
// CHECK-NEXT:  ^bb1(%i: !riscv.reg):
// CHECK-NEXT:    riscv.label "scf_body_0_for"
// CHECK-NEXT:    "test.op"(%i) : (!riscv.reg) -> ()
// CHECK-NEXT:    %1 = riscv.addi %i, 1 : (!riscv.reg) -> !riscv.reg
// CHECK-NEXT:    riscv_cf.blt %1 : !riscv.reg, %three : !riscv.reg, ^bb1(%1 : !riscv.reg), ^bb2(%1 : !riscv.reg)
// CHECK-NEXT:  ^bb2(%2: !riscv.reg):
...
// CHECK-NEXT:    %one = rv32.li 1 : !riscv.reg
// CHECK-NEXT:    %0 = riscv.mv %one : (!riscv.reg) -> !riscv.reg
// CHECK-NEXT:    riscv_cf.j ^bb1(%0 : !riscv.reg) attributes {comment = "Constant folded riscv_cf.bge"}
// CHECK-NEXT:  ^bb1(%1: !riscv.reg):
// CHECK-NEXT:    riscv.label "scf_body_end_0_for"
// CHECK-NEXT:    riscv_func.return
// CHECK-NEXT:  }
