...
def test_query_builder():
    assert list(add_zero_query.variables) == ["root", "0", "1", "rhs_input"]
    assert list(add_zero_query.variables.values()) == [
        OperationVariable("root"),
        SSAValueVariable("0"),
        OperationVariable("1"),
        OperationVariable("rhs_input"),
    ]
    (root_var, root_rhs, root_rhs_op, rhs_input) = add_zero_query.variables.values()
...
# def add_zero(
#     match: bool, rewriter: PatternRewriter, root: arith.Addi, rhs_input: arith.Constant
# ):
#     if match:
#         assert root.rhs == rhs_input.result
#         assert rhs_input.value == IntegerAttr.from_int_and_width(0, 32)
#     else:
#         rewriter.replace_matched_op((), (root.lhs,))
