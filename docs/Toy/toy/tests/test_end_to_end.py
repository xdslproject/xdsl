...
@Builder.implicit_region
def llvm_0():
    main_type = llvm.LLVMFunctionType([], llvm.LLVMVoidType())
    ...
    llvm.FuncOp("main", main_type, body=main)
...
def risc_0():
    ...
    def text_region():
        ...
        def main() -> None:
            ...
            riscv.SwOp(t0, ts1, 0, comment='Set "toy_tensor" @ 0')
            riscv.SwOp(t0, td1, 4, comment='Set "toy_tensor" @ 1')

            riscv.CustomAssemblyInstructionOp("tensor.print", (t0,), ())
            td2 = riscv.LiOp("main.4").rd
            result_vector = riscv.CustomAssemblyInstructionOp(
                "vector.copy",
                (td0,),
            ...
            riscv.SwOp(t1, ts0, 0, comment='Set "toy_tensor" @ 0')
            riscv.SwOp(t1, result_vector, 4, comment='Set "toy_tensor" @ 1')

            riscv.CustomAssemblyInstructionOp("tensor.print", (t1,), ())
            riscv_func.ReturnOp(())
...
main:
    ...
    addi sp, sp, -8
    sw j3, 0(sp)                                 # Set "toy_tensor" @ 0
    sw j4, 4(sp)                                 # Set "toy_tensor" @ 1
    tensor.print sp
    li j5, main.4
    vector.copy j6, j2
    vector.add j6, j5
    # Reserve 8 bytes on stack for element type "toy_tensor"
    addi sp, sp, -8
    sw j1, 0(sp)                                 # Set "toy_tensor" @ 0
    sw j6, 4(sp)                                 # Set "toy_tensor" @ 1
    tensor.print sp
    li a7, 93
    ecall
    ret
...
def test_lower_riscv_func():
    ...
    LowerRISCVFunc(insert_exit_syscall=True).apply(ctx, copy)
    RISCVRegisterAllocation().apply(ctx, copy)

    assert riscv_asm == riscv.riscv_code(copy)
