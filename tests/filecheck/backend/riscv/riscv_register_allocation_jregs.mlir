...
// CHECK-NEXT:      %{{\d+}} = riscv.add %{{\d+}}, %{{\d+}} : (!riscv.reg<j6>, !riscv.reg<s0>) -> !riscv.reg<j1>
// CHECK-NEXT:      "riscv_scf.for"(%{{\d+}}, %{{\d+}}, %{{\d+}}) ({
// CHECK-NEXT:      ^0(%{{\d+}} : !riscv.reg<j2>):
// CHECK-NEXT:        "riscv_scf.yield"() : () -> ()
// CHECK-NEXT:      }) : (!riscv.reg<j6>, !riscv.reg<s0>, !riscv.reg<j1>) -> ()
// CHECK-NEXT:      %{{\d+}} = "riscv_scf.for"(%{{\d+}}, %{{\d+}}, %{{\d+}}, %{{\d+}}) ({
// CHECK-NEXT:      ^1(%{{\d+}} : !riscv.reg<j0>, %{{\d+}} : !riscv.reg<j1>):
// CHECK-NEXT:        %{{\d+}} = riscv.mv %9 : (!riscv.reg<j1>) -> !riscv.reg<j1>
// CHECK-NEXT:        "riscv_scf.yield"(%{{\d+}}) : (!riscv.reg<j1>) -> ()
// CHECK-NEXT:      }) : (!riscv.reg<j6>, !riscv.reg<s0>, !riscv.reg<j1>, !riscv.reg<j1>) -> !riscv.reg<j1>
