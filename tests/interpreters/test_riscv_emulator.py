...
def test_multiply_add():
    ...
    def module():
        @Builder.implicit_region
        def main():
            riscv.LiOp(3, rd=riscv.IntegerRegisters.A0)
            riscv.LiOp(2, rd=riscv.IntegerRegisters.A1)
            riscv.LiOp(1, rd=riscv.IntegerRegisters.A2)

            riscv.JalOp("muladd")
            res = riscv.GetRegisterOp(riscv.IntegerRegisters.A0).res
            riscv.CustomAssemblyInstructionOp("print", [res], [])

            riscv.LiOp(93, rd=riscv.IntegerRegisters.A7)
            riscv.EcallOp()
        ...
        def multiply():
            ...
            a0_multiply = riscv.GetRegisterOp(riscv.IntegerRegisters.A0)
            a1_multiply = riscv.GetRegisterOp(riscv.IntegerRegisters.A1)
            riscv.MulOp(a0_multiply, a1_multiply, rd=riscv.IntegerRegisters.A0)
            riscv.ReturnOp()
        ...
        def add():
            riscv.CommentOp("no extra registers needed, so no need to deal with stack")
            a0_add = riscv.GetRegisterOp(riscv.IntegerRegisters.A0)
            a1_add = riscv.GetRegisterOp(riscv.IntegerRegisters.A1)
            riscv.AddOp(a0_add, a1_add, rd=riscv.IntegerRegisters.A0)
            riscv.ReturnOp()
        ...
        def muladd():
            ...
            a2_muladd = riscv.GetRegisterOp(riscv.IntegerRegisters.A2)

            # get registers we'll use in this section
            sp_muladd = riscv.GetRegisterOp(riscv.IntegerRegisters.SP)
            s0_muladd_0 = riscv.GetRegisterOp(riscv.IntegerRegisters.S0)
            ra_muladd = riscv.GetRegisterOp(riscv.IntegerRegisters.RA)
            riscv.CommentOp(
                "decrement stack pointer by number of register values we need to store for later"
            )
            riscv.AddiOp(sp_muladd, -8, rd=riscv.IntegerRegisters.SP)
