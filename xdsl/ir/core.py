...
class IRWithUses(ABC):
    ...
    def has_uses(self) -> bool:
        """Returns true if the value has uses."""
        return self.first_use is not None

    def has_single_use(self) -> bool:
        """Check if the value has exactly one use."""
        first_use = self.first_use
        return first_use is not None and first_use.next_use is None

    def get_single_use(self) -> Use | None:
        """Return the single use of the value if it has exactly one use."""
    ...
    def uses(self) -> Iterator[Use]:
        """
        Return an iterator over all the uses of the value.
        This should be used when iterating over all uses of a value.
        """
        use = self.first_use
    ...
    def uses_copy(self) -> Sequence[Use]:
        """
        Return a list with all the uses of the value.
        This should be used when iterating over all uses of a value while
        modifying the uses list.
        """
        uses = list[Use]()
        use = self.first_use
        while use is not None:
    ...
    def add_use(self, use: Use):
        ...
        first_use = self.first_use
        use.next_use = first_use
        use.prev_use = None
        if first_use is not None:
            first_use.prev_use = use
        self.first_use = use

    def remove_use(self, use: Use):
        """Remove a use of the value."""
        prev_use = use.prev_use
...
class SSAValue(Generic[AttributeCovT], IRWithUses, ABC):
    ...
    def get_single_use_user(self) -> Operation | None:
        """
        If the SSA value has exactly one use, return the operation that uses it.
        """
        if (single_use := self.get_single_use()) is not None:
            return single_use.operation
        return None
...
class OpResult(Generic[AttributeCovT], SSAValue[AttributeCovT]):
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"index: {self.index}, operation: {self.op.name}, "
            f"uses: {len(self.uses_copy())}>"
        )
...
class BlockArgument(Generic[AttributeCovT], SSAValue[AttributeCovT]):
    ...
    def __repr__(self) -> str:
        return f"<{self.__class__.__name__}[{self.type}] index: {self.index}, uses: {len(self.uses_copy())}>"
