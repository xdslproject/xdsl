...
%expanded2 = tensor.expand_shape %cast1 [[0], [1, 2]] output_shape [4, 2, 2] : tensor<4x4xf32> into tensor<4x2x2xf32>

%concatenated1 = tensor.concat dim(2) %expanded1, %expanded_with_attr_dict : (tensor<?x1x1x?xf32>, tensor<?x1x1x?xf32>) -> tensor<?x1x2x?xf32>
%concatenated2 = tensor.concat dim(1) %tensor, %cast1 {test_attr = 1000 : i32} : (tensor<?x?xf32>, tensor<4x4xf32>) -> tensor<?x?xf32>
%concatenated3 = tensor.concat dim(1) %res_collapse1, %cast1 : (tensor<4x1xf32>, tensor<4x4xf32>) -> tensor<4x5xf32>
...
// CHECK-NEXT:   %expanded2 = tensor.expand_shape %cast1 [[0 : i64], [1 : i64, 2 : i64]] output_shape [4, 2, 2] : tensor<4x4xf32> into tensor<4x2x2xf32>
// CHECK-NEXT:   %concatenated1 = tensor.concat dim(2) %expanded1, %expanded_with_attr_dict : (tensor<?x1x1x?xf32>, tensor<?x1x1x?xf32>) -> tensor<?x1x2x?xf32>
// CHECK-NEXT:   %concatenated2 = tensor.concat dim(1) %tensor, %cast1 {test_attr = 1000 : i32} : (tensor<?x?xf32>, tensor<4x4xf32>) -> tensor<?x?xf32>
// CHECK-NEXT:   %concatenated3 = tensor.concat dim(1) %res_collapse1, %cast1 : (tensor<4x1xf32>, tensor<4x4xf32>) -> tensor<4x5xf32>
...
// CHECK-GENERIC-NEXT:   %expanded2 = "tensor.expand_shape"(%cast1) <{reassociation = [[0 : i64], [1 : i64, 2 : i64]], static_output_shape = array<i64: 4, 2, 2>}> : (tensor<4x4xf32>) -> tensor<4x2x2xf32>
// CHECK-GENERIC-NEXT:   %concatenated1 = "tensor.concat"(%expanded1, %expanded_with_attr_dict) <{dim = 2 : i64}> : (tensor<?x1x1x?xf32>, tensor<?x1x1x?xf32>) -> tensor<?x1x2x?xf32>
...
// CHECK-GENERIC-NEXT:   %concatenated3 = "tensor.concat"(%res_collapse1, %cast1) <{dim = 1 : i64}> : (tensor<4x1xf32>, tensor<4x4xf32>) -> tensor<4x5xf32>
