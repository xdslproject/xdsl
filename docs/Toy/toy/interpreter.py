...
class ToyFunctionTable(InterpreterFunctionTable):
    ...
    def run_toy_func(self, interpreter: Interpreter, name: str,
        ...
        raise InterpretationError(
            f'Could not find toy function with name: {name}')

    @impl(toy.PrintOp)
    def run_print(self, interpreter: Interpreter, op: toy.PrintOp,
                  args: tuple[Any, ...]) -> tuple[Any, ...]:
        interpreter.print(f'{args[0]}')
        return ()

    @impl(toy.FuncOp)
    def run_func(self, interpreter: Interpreter, op: toy.FuncOp,
                 args: tuple[Any, ...]) -> tuple[Any, ...]:
    ...
    @impl(toy.ConstantOp)
    def run_const(self, interpreter: Interpreter, op: toy.ConstantOp,
                  args: tuple[Any, ...]) -> tuple[Any, ...]:
        assert not len(args)
        data = op.get_data()
        shape = op.get_shape()
    ...
    @impl(toy.ReshapeOp)
    def run_reshape(self, interpreter: Interpreter, op: toy.ReshapeOp,
                    args: tuple[Any, ...]) -> tuple[Any, ...]:
        arg, = args
    ...
    @impl(toy.AddOp)
    def run_add(self, interpreter: Interpreter, op: toy.AddOp,
                args: tuple[Any, ...]) -> tuple[Any, ...]:
        lhs, rhs = args
    ...
    @impl(toy.MulOp)
    def run_mul(self, interpreter: Interpreter, op: toy.MulOp,
                args: tuple[Any, ...]) -> tuple[Any, ...]:
        lhs, rhs = args
        assert isinstance(lhs, Tensor)
        assert isinstance(rhs, Tensor)
    ...
    @impl(toy.ReturnOp)
    def run_return(self, interpreter: Interpreter, op: toy.ReturnOp,
                   args: tuple[Any, ...]) -> tuple[Any, ...]:
        assert len(args) < 2
        return ()

    @impl(toy.GenericCallOp)
    def run_generic_call(self, interpreter: Interpreter, op: toy.GenericCallOp,
                         args: tuple[Any, ...]) -> tuple[Any, ...]:
        return self.run_toy_func(interpreter, op.callee.string_value(), args)

    @impl(toy.TransposeOp)
    def run_transpose(self, interpreter: Interpreter, op: toy.TransposeOp,
                      args: tuple[Any, ...]) -> tuple[Any, ...]:
        arg, = args
        assert isinstance(arg, Tensor)
