...
class Data(Attribute, ABC, Generic[DataElement]):
    ...
    @classmethod
    def new(cls: type[Self], params: object) -> Self:
        """
...
class IRWithName(ABC):
    ...
    @name_hint.setter
    def name_hint(self, name: str | None) -> None:
        self._name = self.extract_valid_name(name)

    @classmethod
    def is_valid_name(cls, name: str | None) -> bool:
        """
        Returns `True` if `name` is None or a valid name for an identifier.
        """
        return name is None or _VALUE_NAME_PATTERN.fullmatch(name) is not None
...
class SSAValues(tuple[SSAValueCovT, ...], Generic[SSAValueCovT]):
    ...
    @property
    def types(self) -> tuple[Attribute, ...]:
...
class OpTraits(Iterable[OpTrait]):
    ...
    def add_trait(self, trait: OpTrait) -> None:
...
class Operation(_IRNode):
    ...
    @operands.setter
    def operands(self, new: Sequence[SSAValue]) -> None:
        new = SSAValues(new)
    ...
    @successors.setter
    def successors(self, new: Sequence[Block]) -> None:
        new = tuple(new)
        new_uses = tuple(Use(self, idx) for idx in range(len(new)))
