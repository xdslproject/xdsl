...
def test_parallel_async_only_bare_form():
    """async_only=[#none] prints as the bare `async` keyword, no parens."""
    none = acc.DeviceTypeAttr(acc.DeviceType.NONE)
    op = acc.ParallelOp(
        region=Region(Block([acc.YieldOp()])),
        async_only=ArrayAttr([none]),
    )
    op.verify()
    assert op.async_only == ArrayAttr([none])
...
def test_parallel_async_keyword_only_with_dt():
    """async_only=[#nvidia] prints as `async([#acc.device_type<nvidia>])`."""
    nvidia = acc.DeviceTypeAttr(acc.DeviceType.NVIDIA)
    op = acc.ParallelOp(
        region=Region(Block([acc.YieldOp()])),
        async_only=ArrayAttr([nvidia]),
    )
    op.verify()

    out = io.StringIO()
    Printer(stream=out).print_op(op)
    assert "async([#acc.device_type<nvidia>])" in out.getvalue()


def test_parallel_async_operand_without_dt_prop():
    """Operands without asyncOperandsDeviceType fall back to per-operand #none."""
    a = ConstantOp.from_int_and_width(1, i32)
    op = acc.ParallelOp(
        region=Region(Block([acc.YieldOp()])),
        async_operands=[a.result],
    )
    op.verify()
    assert op.async_operands_device_type is None
    ...
    out = io.StringIO()
    Printer(stream=out).print_op(op)
    assert "async(%0 : i32)" in out.getvalue()


def test_parallel_async_mixed_keyword_and_operand():
    """Both async_only and async_operands populate the mixed pretty form."""
    nvidia = acc.DeviceTypeAttr(acc.DeviceType.NVIDIA)
    default = acc.DeviceTypeAttr(acc.DeviceType.DEFAULT)
    a = ConstantOp.from_int_and_width(1, i32)
    op = acc.ParallelOp(
        region=Region(Block([acc.YieldOp()])),
        async_operands=[a.result],
        async_operands_device_type=ArrayAttr([default]),
        async_only=ArrayAttr([nvidia]),
    )
    op.verify()
    ...
    Printer(stream=out).print_op(op)
    assert (
        "async([#acc.device_type<nvidia>], %0 : i32 [#acc.device_type<default>])"
        in out.getvalue()
    )
