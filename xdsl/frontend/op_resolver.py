...
class OpResolver:
    ...
    def resolve_op(module_name: str, func_name: str) -> Callable[..., Operation]:
        ...
        if not hasattr(module, resolver_name):
            raise FrontendProgramException(f"Internal failure: cannot resolve operation '{func_name}' from module '{module_name}'.")
    ...
    def resolve_op_overload(python_op: str, frontend_type: Type[_FrontendType]) -> Callable[..., Operation]:
        ...
        if not hasattr(frontend_type , python_op):
            raise FrontendProgramException(f"Internal failure: '{frontend_type.__name__}' does not overload '{python_op}'.")
        ...
        if not isinstance(python_ast, ast.Module) or not isinstance(python_ast.body[0], ast.FunctionDef):
            raise FrontendProgramException(f"Internal failure while resolving '{python_op}'. Function AST for resolution not found.")
