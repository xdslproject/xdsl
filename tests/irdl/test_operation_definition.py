...
def test_default_accessors():
    ...
    assert parsed.prop.value.data is False

    assert parsed.properties.get("opt_prop") is None

    assert parsed.opt_prop.value.data is True

    assert parsed.attr.value.data is False

    assert parsed.attributes.get("opt_attr") is None

    assert parsed.opt_attr.value.data is True
