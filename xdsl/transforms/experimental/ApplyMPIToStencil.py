...
from xdsl.dialects import builtin, func, llvm, arith, mpi, memref, scf
...
class ApplyMPIToExternalLoad(RewritePattern):
  ...
  def match_and_rewrite(self, op: stencil.ExternalLoadOp, rewriter: PatternRewriter, /):
    ...
    dim_zero_const=arith.Constant.from_attr(builtin.IntegerAttr(0, builtin.IndexType()), builtin.IndexType())
    ...
    mpi_operations+=[datatype_op, dim_zero_const, dim_zero_size_op, dim_zero_i32_op, dim_zero_i64_op, index_memref, index_memref_i64]
    ...
    mpi_operations+=[alloc_lookup_op_zero, alloc_lookup_op_one, alloc_lookup_op_two, alloc_lookup_op_three, mpi_request_null]
    ...
    mpi_send_top_op=mpi.Isend.get(send_ptr, dim_zero_i32_op, datatype_op, rank_m1_op, zero, alloc_lookup_op_zero)
    
    recv_ptr=llvm.IntToPtrOp.get(index_memref_i64)
    mpi_recv_top_op=mpi.Irecv.get(recv_ptr, dim_zero_i32_op, datatype_op, rank_m1_op, zero, alloc_lookup_op_one)
    
    # Else set empty request handles
    zero_conv=builtin.UnrealizedConversionCastOp.get([alloc_lookup_op_zero], [llvm.LLVMPointerType.typed(builtin.i32)])
    null_req_zero=llvm.StoreOp.get(mpi_request_null, zero_conv)
    one_conv=builtin.UnrealizedConversionCastOp.get([alloc_lookup_op_one], [llvm.LLVMPointerType.typed(builtin.i32)])
    null_req_one=llvm.StoreOp.get(mpi_request_null, one_conv)

    top_halo_exhange=scf.If.get(compare_top_op, [], [rank_m1_op, add_offset, 
                      added_ptr, send_ptr, mpi_send_top_op, recv_ptr, mpi_recv_top_op, scf.Yield.get()], 
    ...
    mpi_send_bottom_op=mpi.Isend.get(ptr_b_send, dim_zero_i32_op, datatype_op, rank_p1_op, zero, alloc_lookup_op_two)
    ...
    mpi_recv_bottom_op=mpi.Irecv.get(ptr_b_recv, dim_zero_i32_op, datatype_op, rank_p1_op, zero, alloc_lookup_op_three)
    
    # Else set empty request handles
    two_conv=builtin.UnrealizedConversionCastOp.get([alloc_lookup_op_two], [llvm.LLVMPointerType.typed(builtin.i32)])
    null_req_two=llvm.StoreOp.get(mpi_request_null, two_conv)
    three_conv=builtin.UnrealizedConversionCastOp.get([alloc_lookup_op_three], [llvm.LLVMPointerType.typed(builtin.i32)])
    null_req_three=llvm.StoreOp.get(mpi_request_null, three_conv)
    
    bottom_halo_exhange=scf.If.get(compare_bottom_op, [], [rank_p1_op, col_row_b_send, 
      element_b_send, add_offset_b_send, added_ptr_b_send, ptr_b_send, mpi_send_bottom_op, 
      col_row_b_recv, element_b_recv, add_offset_b_recv, added_ptr_b_recv, ptr_b_recv, mpi_recv_bottom_op, scf.Yield.get()], 
...
def ApplyMpi(ctx: MLContext, module: builtin.ModuleOp):
