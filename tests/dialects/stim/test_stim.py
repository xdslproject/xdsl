...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            '"stim.assign_qubit_coord"() <{"qubitmapping" = #stim.qubit_coord<(0,0), !stim.qubit<0>>}> : () -> ()',
        ),
    ],
)
def test_stim_qubit_coord(program: str, generic_program: str):
    """Check that the given program roundtrips exactly (including whitespaces)."""
    check_roundtrip(program)
    check_equivalence(program, generic_program)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            '"stim.circuit"() ({stim.assign_qubit_coord <(0, 0), !stim.qubit<0>>}) {"hello" = "world"} : () -> ()',
        ),
        (
            'stim.circuit qubitlayout [#stim.qubit_coord<(0, 0), !stim.qubit<0>>] attributes {"hello" = "world"} {\n  stim.assign_qubit_coord <(1, 2), !stim.qubit<2>>\n}',
            '"stim.circuit"() <{"qubitlayout" = [#stim.qubit_coord<(0, 0), !stim.qubit<0>>]}> ({stim.assign_qubit_coord <(1, 2), !stim.qubit<2>>}) {"hello" = "world"} : () -> ()',
        ),
    ],
)
def test_stim_circuit(program: str, generic_program: str):
    check_roundtrip(program)
    check_equivalence(program, generic_program)
