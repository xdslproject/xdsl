...
def simple_linear_riscv_allocated():
    ...
    def text_region():
        ...
        def main_region() -> None:
            zero = riscv.GetRegisterOp(riscv.Registers.ZERO).res
            v0 = riscv.AddiOp(zero, 1, rd=riscv.Registers.A6).rd
            v1 = riscv.AddiOp(zero, 2, rd=riscv.Registers.A7).rd

            v3 = riscv.AddOp(v1, v0, rd=riscv.Registers.S2).rd
            v4 = riscv.MulOp(v3, v1, rd=riscv.Registers.S3).rd
            v5 = riscv.SubOp(v0, v4, rd=riscv.Registers.S4).rd
            v6 = riscv.MulOp(v1, v0, rd=riscv.Registers.S5).rd
            v7 = riscv.AddOp(v6, v3, rd=riscv.Registers.S6).rd
            v8 = riscv.AddOp(v7, v5, rd=riscv.Registers.S7).rd
            v9 = riscv.SubOp(v0, v1, rd=riscv.Registers.S8).rd
            v10 = riscv.MulOp(v5, v3, rd=riscv.Registers.S9).rd
            v11 = riscv.AddOp(v9, v10, rd=riscv.Registers.S10).rd
            v12 = riscv.AddOp(v11, v8, rd=riscv.Registers.S11).rd
