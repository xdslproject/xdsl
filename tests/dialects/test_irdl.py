...
def test_dialect_accessors():
    ...
    """
    type1 = TypeOp.create(attributes={"name": StringAttr("type1")}, regions=[Region()])
    type2 = TypeOp.create(attributes={"name": StringAttr("type2")}, regions=[Region()])
    op1 = OperationOp.create(attributes={"name": StringAttr("op1")}, regions=[Region()])
    op2 = OperationOp.create(attributes={"name": StringAttr("op2")}, regions=[Region()])
...
def test_operation_accessors():
    ...
    empty_op = OperationOp.create(
        attributes={"name": StringAttr("op")}, regions=[Region([Block()])]
    )
