from xdsl.dialects import arith, builtin, func, gpu, memref
...
builtin.module attributes {gpu.container_module} {
  "gpu.module"() ({
    ...
    ^0(%arg : memref<4x4xf32>):
      ...
      "memref.store"(%5, %arg, %1, %2) {"nontemporal" = false} : (index, memref<4x4xf32>, index, index) -> ()
      "gpu.return"() : () -> ()
    }) {"function_type" = (memref<4x4xf32>) -> (),
        "gpu.kernel",
        "sym_name" = "fill"
       } : () -> ()
    "gpu.module_end"() : () -> ()
  ...
  func.func @main() -> index {
    ...
    %memref = "memref.alloc"() {"alignment" = 0 : i64, "operand_segment_sizes" = array<i32: 0, 0>} : () -> memref<4x4xf32>
    "gpu.launch_func"(%four, %four, %one, %one, %one, %one, %memref) {"operand_segment_sizes" = array<i32: 0, 1, 1, 1, 1, 1, 1, 1, 1>, "kernel" = @gpu::@fill} : (index, index, index, index, index, index, memref<4x4xf32>) -> ()
  }
}
"""
    context = MLContext()
