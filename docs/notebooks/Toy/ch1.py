...
def _(mo):
    mo.md(r"""
    You can reproduce this result by parsing the same program inline below
    (equivalent CLI: `toyc docs/Toy/examples/ast.toy --emit=ast`):
    """)
    return
...
def _():
    ...
    # Same source as docs/Toy/examples/ast.toy (without lit RUN/CHECK directives).
    ast_toy = """
    # User defined generic function that operates on unknown shaped arguments.
    def multiply_transpose(a, b) {
      return transpose(a) * transpose(b);
    ...
    parser = ToyParser(Path("examples/ast.toy"), ast_toy)
...
def _(mo):
    ...
    `toy/frontend/lexer.py`. The parser can be found in `toy/frontend/parser.py`; it is a recursive
