...
def test_all_reduce():
    ...
    def body():
        sum = Operation.clone(arith.Addi(*body_block.args))
        YieldOp.get([sum])
...
def test_launch():
    ...
    nd_launch = LaunchOp.get(body2, gridSize, blockSize, True, asyncDependencies, ten)
