...
class DLTSelectRewriter(RewritePattern):
    ...
    def match_and_rewrite(self, select: dlt.SelectOp, rewriter: PatternRewriter):
        ...
        dim_map |= {
            dim: PtrCarriedGetter(llvm_in, input_type, dim=dim)
            for dim in input_type.filled_dimensions
        }

        extent_map = {
            extent: PtrCarriedGetter(llvm_in, input_type, extent=extent)
        ...
        select_ops, select_result = get_select_for(
            input_layout,
            output_layout,
            members,
            dim_map,
...
def get_select_for(
    ...
    raise NotImplementedError(
        f"get_select_for not implemented for this layout: {type(starting_layout)} : {starting_layout}"
    )
...
@get_select_for.register
def _(
    starting_layout: dlt.NamedLayoutAttr,
    ending_layout: dlt.Layout,
    members: set[dlt.MemberAttr],
    dim_mapping: dict[dlt.DimensionAttr, IndexGetter],
    extent_resolver: ExtentResolver,
...
def _(
    ...
    for i, child_layout in enumerate(starting_layout.children):
        ...
        if child_layout.contents_type.has_selectable(members, dim_mapping.keys()):
            ...
            child = child_layout
            assert not any(
                c.contents_type.has_selectable(members, dim_mapping.keys())
                for _i, c in enumerate(starting_layout.children)
                if _i > i
            )
            break
        else:
    ...
    if offset > 0:
        if offset_ssa is not None:
            offset_ops.append(
                const_offset := arith.Constant(IntegerAttr(offset, IndexType()))
            )
        ...
        if offset_ssa is None:
            return get_select_for(
                child, ending_layout, members, dim_mapping, extent_resolver, input
            )

    ptr_ops, ptr = add_to_llvm_pointer(input, offset_ssa)
    child_ops, child_res = get_select_for(
...
def _(
    ...
    size_ops, size, extra = from_int_to_ssa(
        get_size_from_layout(child_layout, extent_resolver)
    )
...
class DLTGetRewriter(RewritePattern):
    ...
    def match_and_rewrite(self, get_op: dlt.GetOp, rewriter: PatternRewriter):
        ...
        if not isinstance(input_type.layout, dlt.PrimitiveLayoutAttr):
            select_op = dlt.SelectOp(
                ...
                input_type.with_new_layout(
                    dlt.PrimitiveLayoutAttr(base_type=get_type), remove_bloat=True
                ),
            )
        ...
        cast_input_op = UnrealizedConversionCastOp.get(
            input_ptr, get_llvm_type_from_dlt_ptr(input_type)
        )
        ops.append(cast_input_op)
        llvm_in = cast_input_op.outputs[0]
    ...
    def match_and_rewrite(self, set_op: dlt.SetOp, rewriter: PatternRewriter):
        ...
        if not isinstance(input_type.layout, dlt.PrimitiveLayoutAttr):
            select_op = dlt.SelectOp(
                ...
                [],
                input_type.with_new_layout(
                    dlt.PrimitiveLayoutAttr(base_type=set_type), remove_bloat=True
                ),
            )
            ops.append(select_op)
            input_ptr = select_op.res
            ...
            for extent, ext_dims in zip(iterate_op.extents, tensor_dims):
                ...
                if extent.is_static():
                    ...
                    get_tensor_extent_ops, tensor_extent = (
                        tensor_arg_extent_resolver.resolve(extent)
                    )
...
class DLTExtractExtentRewriter(RewritePattern):
    ...
    def match_and_rewrite(
        ...
        e_map = {
            extent: PtrCarriedGetter(cast_op.outputs[0], dlt_ptr, extent=extent)
            for extent in dlt_ptr.filled_extents
        }
        extent_resolver = ExtentResolver(e_map)

        resolve_ops, extent_ssa = from_int_to_ssa(
            extent_resolver.resolve(extract_op.extent)
        )
