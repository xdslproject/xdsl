...
// CHECK-NEXT:    %0 = builtin.unrealized_conversion_cast %m_f32 : memref<3x2xf32> to index
...
// CHECK-NEXT:    riscv.fsw %offset_pointer_1, %v_f32_1, 0 {"comment" = "store float value to pointer"} : (!riscv.reg, !riscv.freg) -> ()
...
// CHECK-NEXT:    %scaled_pointer_offset_1 = riscv.mul %pointer_dim_stride_5, %bytes_per_element_1 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %x_f32 = builtin.unrealized_conversion_cast %m_f32 : memref<3x2xf32> to index
// CHECK-NEXT:    %offset_pointer_2 = builtin.unrealized_conversion_cast %x_f32 : index to !riscv.reg
// CHECK-NEXT:    %offset_pointer_3 = riscv.add %offset_pointer_2, %scaled_pointer_offset_1 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %x_f32_1 = riscv.flw %offset_pointer_3, 0 {"comment" = "load float from pointer"} : (!riscv.reg) -> !riscv.freg
%x_f32 = memref.load %m_f32[%r, %c] {"nontemporal" = false} : memref<3x2xf32>

// CHECK-NEXT:    %bytes_per_element_2 = riscv.li 4 : !riscv.reg
// CHECK-NEXT:    %scaled_pointer_offset_2 = builtin.unrealized_conversion_cast %c : index to !riscv.reg
// CHECK-NEXT:    %scaled_pointer_offset_3 = riscv.mul %scaled_pointer_offset_2, %bytes_per_element_2 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %1 = builtin.unrealized_conversion_cast %m_i32 : memref<3xi32> to index
// CHECK-NEXT:    %offset_pointer_4 = builtin.unrealized_conversion_cast %1 : index to !riscv.reg
// CHECK-NEXT:    %offset_pointer_5 = riscv.add %offset_pointer_4, %scaled_pointer_offset_3 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %v_i32_1 = builtin.unrealized_conversion_cast %v_i32 : i32 to !riscv.reg
// CHECK-NEXT:    riscv.sw %offset_pointer_5, %v_i32_1, 0 {"comment" = "store int value to pointer"} : (!riscv.reg, !riscv.reg) -> ()
memref.store %v_i32, %m_i32[%c] {"nontemporal" = false} : memref<3xi32>

// CHECK-NEXT:    %bytes_per_element_3 = riscv.li 4 : !riscv.reg
// CHECK-NEXT:    %scaled_pointer_offset_4 = builtin.unrealized_conversion_cast %c : index to !riscv.reg
// CHECK-NEXT:    %scaled_pointer_offset_5 = riscv.mul %scaled_pointer_offset_4, %bytes_per_element_3 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %x_i32 = builtin.unrealized_conversion_cast %m_i32 : memref<3xi32> to index
// CHECK-NEXT:    %offset_pointer_6 = builtin.unrealized_conversion_cast %x_i32 : index to !riscv.reg
// CHECK-NEXT:    %offset_pointer_7 = riscv.add %offset_pointer_6, %scaled_pointer_offset_5 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %x_i32_1 = riscv.lw %offset_pointer_7, 0 {"comment" = "load word from pointer"} : (!riscv.reg) -> !riscv.reg
...
// CHECK-NEXT:    %scaled_pointer_offset_6 = riscv.mul %pointer_dim_stride_8, %bytes_per_element_4 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %2 = builtin.unrealized_conversion_cast %m_f64 : memref<3x2xf64> to index
// CHECK-NEXT:    %offset_pointer_8 = builtin.unrealized_conversion_cast %2 : index to !riscv.reg
// CHECK-NEXT:    %offset_pointer_9 = riscv.add %offset_pointer_8, %scaled_pointer_offset_6 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %v_f64_1 = builtin.unrealized_conversion_cast %v_f64 : f64 to !riscv.freg
// CHECK-NEXT:    riscv.fsd %offset_pointer_9, %v_f64_1, 0 {"comment" = "store double value to pointer"} : (!riscv.reg, !riscv.freg) -> ()
memref.store %v_f64, %m_f64[%r, %c] {"nontemporal" = false} : memref<3x2xf64>

// CHECK-NEXT:    %scalar_x_i32 = builtin.unrealized_conversion_cast %m_scalar_i32 : memref<i32> to index
// CHECK-NEXT:    %scalar_x_i32_1 = builtin.unrealized_conversion_cast %scalar_x_i32 : index to !riscv.reg
// CHECK-NEXT:    %scalar_x_i32_2 = riscv.lw %scalar_x_i32_1, 0 {"comment" = "load word from pointer"} : (!riscv.reg) -> !riscv.reg
%scalar_x_i32 = memref.load %m_scalar_i32[] {"nontemporal" = false} : memref<i32>

// CHECK-NEXT:    %3 = builtin.unrealized_conversion_cast %m_scalar_i32 : memref<i32> to index
// CHECK-NEXT:    %4 = builtin.unrealized_conversion_cast %3 : index to !riscv.reg
// CHECK-NEXT:    riscv.sw %4, %scalar_x_i32_2, 0 {"comment" = "store int value to pointer"} : (!riscv.reg, !riscv.reg) -> ()
...
// CHECK-NEXT:    %scaled_pointer_offset_7 = riscv.mul %pointer_dim_stride_11, %bytes_per_element_5 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %x_f64 = builtin.unrealized_conversion_cast %m_f64 : memref<3x2xf64> to index
// CHECK-NEXT:    %offset_pointer_10 = builtin.unrealized_conversion_cast %x_f64 : index to !riscv.reg
// CHECK-NEXT:    %offset_pointer_11 = riscv.add %offset_pointer_10, %scaled_pointer_offset_7 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %x_f64_1 = riscv.fld %offset_pointer_11, 0 {"comment" = "load double from pointer"} : (!riscv.reg) -> !riscv.freg
...
// CHECK-NEXT:    %scaled_pointer_offset = builtin.unrealized_conversion_cast %d0 : index to !riscv.reg
// CHECK-NEXT:    %scaled_pointer_offset_1 = riscv.mul %scaled_pointer_offset, %bytes_per_element : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %0 = builtin.unrealized_conversion_cast %m : memref<1xi8> to index
// CHECK-NEXT:    %offset_pointer = builtin.unrealized_conversion_cast %0 : index to !riscv.reg
// CHECK-NEXT:    %offset_pointer_1 = riscv.add %offset_pointer, %scaled_pointer_offset_1 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %v_1 = builtin.unrealized_conversion_cast %v : i8 to !riscv.reg
// CHECK-NEXT:    riscv.sw %offset_pointer_1, %v_1, 0 {"comment" = "store int value to pointer"} : (!riscv.reg, !riscv.reg) -> ()
...
// CHECK-NEXT:    %scaled_pointer_offset = builtin.unrealized_conversion_cast %d0 : index to !riscv.reg
// CHECK-NEXT:    %scaled_pointer_offset_1 = riscv.mul %scaled_pointer_offset, %bytes_per_element : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %0 = builtin.unrealized_conversion_cast %m : memref<1xi16> to index
// CHECK-NEXT:    %offset_pointer = builtin.unrealized_conversion_cast %0 : index to !riscv.reg
// CHECK-NEXT:    %offset_pointer_1 = riscv.add %offset_pointer, %scaled_pointer_offset_1 : (!riscv.reg, !riscv.reg) -> !riscv.reg
...
// CHECK-NEXT:    %scaled_pointer_offset = builtin.unrealized_conversion_cast %d0 : index to !riscv.reg
// CHECK-NEXT:    %scaled_pointer_offset_1 = riscv.mul %scaled_pointer_offset, %bytes_per_element : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:    %0 = builtin.unrealized_conversion_cast %m : memref<1xi64> to index
// CHECK-NEXT:    %offset_pointer = builtin.unrealized_conversion_cast %0 : index to !riscv.reg
// CHECK-NEXT:    %offset_pointer_1 = riscv.add %offset_pointer, %scaled_pointer_offset_1 : (!riscv.reg, !riscv.reg) -> !riscv.reg
