...
// CHECK-NEXT:   %0 = builtin.unrealized_conversion_cast %m_f32 : memref<3x2xf32> to !riscv.reg
// CHECK-NEXT:   %offset_pointer = riscv.add %0, %scaled_pointer_offset : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %v_f32_1 = builtin.unrealized_conversion_cast %v_f32 : f32 to !riscv.freg
// CHECK-NEXT:   riscv.fsw %offset_pointer, %v_f32_1, 0 {comment = "store float value to pointer"} : (!riscv.reg, !riscv.freg) -> ()
...
// CHECK-NEXT:   %x_f32 = builtin.unrealized_conversion_cast %m_f32 : memref<3x2xf32> to !riscv.reg
// CHECK-NEXT:   %offset_pointer_1 = riscv.add %x_f32, %scaled_pointer_offset_1 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %x_f32_1 = riscv.flw %offset_pointer_1, 0 {comment = "load float from pointer"} : (!riscv.reg) -> !riscv.freg
...
// CHECK-NEXT:   %1 = builtin.unrealized_conversion_cast %m_i32 : memref<3xi32> to !riscv.reg
// CHECK-NEXT:   %offset_pointer_2 = riscv.add %1, %scaled_pointer_offset_3 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %v_i32_1 = builtin.unrealized_conversion_cast %v_i32 : i32 to !riscv.reg
// CHECK-NEXT:   riscv.sw %offset_pointer_2, %v_i32_1, 0 {comment = "store int value to pointer"} : (!riscv.reg, !riscv.reg) -> ()
...
// CHECK-NEXT:   %x_i32 = builtin.unrealized_conversion_cast %m_i32 : memref<3xi32> to !riscv.reg
// CHECK-NEXT:   %offset_pointer_3 = riscv.add %x_i32, %scaled_pointer_offset_5 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %x_i32_1 = riscv.lw %offset_pointer_3, 0 {comment = "load word from pointer"} : (!riscv.reg) -> !riscv.reg
...
// CHECK-NEXT:   %2 = builtin.unrealized_conversion_cast %m_f64 : memref<3x2xf64> to !riscv.reg
// CHECK-NEXT:   %offset_pointer_4 = riscv.add %2, %scaled_pointer_offset_6 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %v_f64_1 = builtin.unrealized_conversion_cast %v_f64 : f64 to !riscv.freg
// CHECK-NEXT:   riscv.fsd %offset_pointer_4, %v_f64_1, 0 {comment = "store double value to pointer"} : (!riscv.reg, !riscv.freg) -> ()
memref.store %v_f64, %m_f64[%r, %c] {"nontemporal" = false} : memref<3x2xf64>

// CHECK-NEXT:   %scalar_x_i32 = builtin.unrealized_conversion_cast %m_scalar_i32 : memref<i32> to !riscv.reg
// CHECK-NEXT:   %scalar_x_i32_1 = riscv.lw %scalar_x_i32, 0 {comment = "load word from pointer"} : (!riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %3 = builtin.unrealized_conversion_cast %m_scalar_i32 : memref<i32> to !riscv.reg
%scalar_x_i32 = memref.load %m_scalar_i32[] {"nontemporal" = false} : memref<i32>

// CHECK-NEXT:   riscv.sw %3, %scalar_x_i32_1, 0 {comment = "store int value to pointer"} : (!riscv.reg, !riscv.reg) -> ()
...
// CHECK-NEXT:   %x_f64 = builtin.unrealized_conversion_cast %m_f64 : memref<3x2xf64> to !riscv.reg
// CHECK-NEXT:   %offset_pointer_5 = riscv.add %x_f64, %scaled_pointer_offset_7 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %x_f64_1 = riscv.fld %offset_pointer_5, 0 {comment = "load double from pointer"} : (!riscv.reg) -> !riscv.freg
...
// CHECK-NEXT:  %0 = builtin.unrealized_conversion_cast %m : memref<1xi8> to !riscv.reg
// CHECK-NEXT:  %offset_pointer = riscv.add %0, %scaled_pointer_offset_1 : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:  %v_1 = builtin.unrealized_conversion_cast %v : i8 to !riscv.reg
// CHECK-NEXT:  riscv.sw %offset_pointer, %v_1, 0 {comment = "store int value to pointer"} : (!riscv.reg, !riscv.reg) -> ()
...
// CHECK-NEXT:  %scaled_pointer_offset = builtin.unrealized_conversion_cast %d0 : index to !riscv.reg
// CHECK-NEXT:  %scaled_pointer_offset_1 = riscv.mul %scaled_pointer_offset, %bytes_per_element : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:  %0 = builtin.unrealized_conversion_cast %m : memref<1xi16> to !riscv.reg
// CHECK-NEXT:  %offset_pointer = riscv.add %0, %scaled_pointer_offset_1 : (!riscv.reg, !riscv.reg) -> !riscv.reg
...
// CHECK-NEXT:  %scaled_pointer_offset = builtin.unrealized_conversion_cast %d0 : index to !riscv.reg
// CHECK-NEXT:  %scaled_pointer_offset_1 = riscv.mul %scaled_pointer_offset, %bytes_per_element : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:  %0 = builtin.unrealized_conversion_cast %m : memref<1xi64> to !riscv.reg
// CHECK-NEXT:  %offset_pointer = riscv.add %0, %scaled_pointer_offset_1 : (!riscv.reg, !riscv.reg) -> !riscv.reg
...
// CHECK-NEXT:   %bytes_per_element = riscv.li 8 : !riscv.reg
// CHECK-NEXT:   %scaled_pointer_offset = riscv.mul %pointer_dim_stride_2, %bytes_per_element : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %v = builtin.unrealized_conversion_cast %m : memref<2x3xf64, strided<[6, 1], offset: ?>> to !riscv.reg
// CHECK-NEXT:   %offset_pointer = riscv.add %v, %scaled_pointer_offset : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %v_1 = riscv.fld %offset_pointer, 0 {comment = "load double from pointer"} : (!riscv.reg) -> !riscv.freg
...
// CHECK-NEXT:   %bytes_per_element = riscv.li 8 : !riscv.reg
// CHECK-NEXT:   %scaled_pointer_offset = riscv.mul %pointer_dim_stride_2, %bytes_per_element : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %0 = builtin.unrealized_conversion_cast %m : memref<2x3xf64, strided<[6, 1], offset: ?>> to !riscv.reg
// CHECK-NEXT:   %offset_pointer = riscv.add %0, %scaled_pointer_offset : (!riscv.reg, !riscv.reg) -> !riscv.reg
// CHECK-NEXT:   %v_1 = builtin.unrealized_conversion_cast %v : f64 to !riscv.freg
// CHECK-NEXT:   riscv.fsd %offset_pointer, %v_1, 0 {comment = "store double value to pointer"} : (!riscv.reg, !riscv.freg) -> ()
