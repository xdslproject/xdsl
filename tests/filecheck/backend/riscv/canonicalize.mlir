...
builtin.module {
  ...
  %multiply_immediate_power_two = riscv.mul %i1, %c2 : (!riscv.reg<a1>, !riscv.reg) -> !riscv.reg<a0>
  "test.op"(%multiply_immediate_power_two) : (!riscv.reg<a0>) -> ()
...
// CHECK-NEXT:   %multiply_immediate_power_two = riscv.slli %i1, 1 : (!riscv.reg) -> !riscv.reg<a0>
// CHECK-NEXT:   "test.op"(%multiply_immediate_power_two) : (!riscv.reg<a0>) -> ()
