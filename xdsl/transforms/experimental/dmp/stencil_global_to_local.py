...
class ChangeStoreOpSizes(RewritePattern):
    ...
    def match_and_rewrite(self, op: stencil.StoreOp, rewriter: PatternRewriter, /):
        ...
        op.bounds = stencil.StencilBoundsAttr.from_lb_ub(
            stencil.IndexAttr(*(len(new_shape) * [0])),
            stencil.IndexAttr(*new_shape),
        )
...
def generate_memcpy(
    ...
    offset = stencil.IndexAttr(*ex.offset) - field_type.bounds.lb
