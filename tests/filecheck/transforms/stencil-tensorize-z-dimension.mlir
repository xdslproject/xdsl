...
// CHECK-NEXT:          %23 = arith.mulf %22, %5 : tensor<510xf32>
...
// CHECK-NEXT:         %21 = arith.mulf %20, %3 : tensor<510xf32>
...
// CHECK-NEXT:     }
  ...
  func.func @dmp_swap(%a : !stencil.field<[-1,1023]x[-1,511]x[-1,511]xf32>, %b : !stencil.field<[-1,1023]x[-1,511]x[-1,511]xf32>) {
    ...
    "test.op"(%1) : (!stencil.temp<[-1,2]x[-1,2]x[-1,511]xf32>) -> ()
    func.return
  }
...
// CHECK-NEXT:    "test.op"(%1) : (!stencil.temp<[-1,2]x[-1,2]xtensor<512xf32>>) -> ()
// CHECK-NEXT:    func.return
// CHECK-NEXT:  }
...
// CHECK-NEXT:        %22 = arith.mulf %21, %4 : tensor<510xf32>
