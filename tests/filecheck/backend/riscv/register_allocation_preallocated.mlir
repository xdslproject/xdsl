// RUN: xdsl-opt -p riscv-allocate-registers{allocation_strategy=BlockNaive} %s --print-op-generic | filecheck %s --check-prefix=BNAIVE
// RUN: xdsl-opt -p "riscv-allocate-registers{allocation_strategy=LivenessBlockNaive limit_registers=3}" %s --print-op-generic | filecheck %s --check-prefix=LIVE-BNAIVE
