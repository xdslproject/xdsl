from xdsl.dialects.experimental.dmp import HaloExchangeDecl, HaloShapeInformation
from xdsl.transforms.experimental.dmp import decompositions


def flat_face_exchanges(
    shape: HaloShapeInformation, dim: int
) -> tuple[HaloExchangeDecl, HaloExchangeDecl]:
    func = (
...
def test_decomp_flat_face_3d():
    shape = HaloShapeInformation.from_index_attrs(
        (0, 0, 0),  # buff lb
        (4, 4, 4),  # core lb
        (14, 14, 14),  # core ub
        (18, 18, 18),  # buff ub
