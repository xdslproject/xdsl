...
def test_parse_optional_bool__int_or_float_numeric(numeric: str, is_int: bool):
...
def test_parse_bool_int_or_float_numeric(numeric: str, is_int: bool):
    parser = Parser(Context(), numeric)
    value_span = parser._parse_bool_int_or_float()
    assert value_span is not None
    value, span = value_span
    caster = int if is_int else float
    assert value == caster(numeric)
