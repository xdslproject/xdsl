...
builtin.module {
  ...
  func.func @omp_target_data_task(%dep: memref<1xi32>, %dev: i32, %if: i1, %m1: memref<1xf32>, %m2: memref<1xf32>, %m3: memref<1xf32>) {
    ...
    %del = "omp.map.info" (%m3) <{operandSegmentSizes = array<i32: 1, 0, 0, 0>, var_type = memref<1xf32>, map_type = #omp<clause_map_flags delete>, map_capture_type = #omp<variable_capture_kind(ByCopy)>}> : (memref<1xf32>) -> memref<1xf32>
...
// CHECK-NEXT:      %{{.*}} = "omp.map.info"(%{{.*}}) <{map_capture_type = #omp<variable_capture_kind (ByCopy)>, map_type = #omp<clause_map_flags delete>, operandSegmentSizes = array<i32: 1, 0, 0, 0>, partial_map = false, var_type = memref<1xf32>}> : (memref<1xf32>) -> memref<1xf32>
