...
builtin.module {
  ...
  %shift_left_zero_r0 = riscv.slli %i2, 0 : (!riscv.reg) -> !riscv.reg
  "test.op"(%shift_left_zero_r0) : (!riscv.reg) -> ()
...
// CHECK-NEXT:   %shift_left_zero_r0 = riscv.mv %i2 : (!riscv.reg) -> !riscv.reg
// CHECK-NEXT:   "test.op"(%shift_left_zero_r0) : (!riscv.reg) -> ()
