...
"builtin.module"() ({
  ...
  // CHECK: %{{.*}} = riscv.get_register : () -> (!riscv.reg<x$>)
  // CHECK-NEXT: %{{.*}} = riscv.get_register : () -> (!riscv.reg<x$>)
  ...
  // CHECK-NEXT: %addi = riscv.addi %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %slti = riscv.slti %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %sltiu = riscv.sltiu %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %andi = riscv.andi %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %ori = riscv.ori %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %xori = riscv.xori %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %slli = riscv.slli %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %srli = riscv.srli %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %srai = riscv.srai %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %lui = riscv.lui 1 : () -> (!riscv.reg<x$>)
  // CHECK-NEXT: %auipc = riscv.auipc 1 : () -> (!riscv.reg<x$>)
  // CHECK-NEXT: %mv = riscv.mv %0 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  ...
  "riscv.nop"() : () -> ()
  // CHECK-NEXT: %add = riscv.add %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %slt = riscv.slt %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %sltu = riscv.sltu %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %and = riscv.and %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %or = riscv.or %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %xor = riscv.xor %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %sll = riscv.sll %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %srl = riscv.srl %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %sub = riscv.sub %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %sra = riscv.sra %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  ...
  // CHECK-NEXT: riscv.jalr %0, 1 : (!riscv.reg<x$>) -> ()
  // CHECK-NEXT: riscv.jalr %0, 1, x$ : (!riscv.reg<x$>) -> ()
  // CHECK-NEXT: riscv.jalr %0, "label" : (!riscv.reg<x$>) -> ()
  ...
  // CHECK-NEXT: riscv.beq %0, %1, 1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> ()
  // CHECK-NEXT: riscv.bne %0, %1, 1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> ()
  // CHECK-NEXT: riscv.blt %0, %1, 1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> ()
  // CHECK-NEXT: riscv.bge %0, %1, 1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> ()
  // CHECK-NEXT: riscv.bltu %0, %1, 1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> ()
  // CHECK-NEXT: riscv.bgeu %0, %1, 1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> ()
  ...
  // CHECK-NEXT: %lb = riscv.lb %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %lbu = riscv.lbu %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %lh = riscv.lh %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %lhu = riscv.lhu %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %lw = riscv.lw %0, 1 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: riscv.sb %0, %1, 1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> ()
  // CHECK-NEXT: riscv.sh %0, %1, 1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> ()
  // CHECK-NEXT: riscv.sw %0, %1, 1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> ()
  ...
  // CHECK-NEXT: %csrrw_rw = riscv.csrrw %0, 1024 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrw_w = riscv.csrrw %0, 1024, w : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrs_rw = riscv.csrrs %0, 1024 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrs_r = riscv.csrrs %0, 1024, r : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrc_rw = riscv.csrrc %0, 1024 : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrc_r = riscv.csrrc %0, 1024, r : (!riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrsi_rw = riscv.csrrsi 1024, 8 : () -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrsi_r = riscv.csrrsi 1024, 0 : () -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrci_rw = riscv.csrrci 1024, 8 : () -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrci_r = riscv.csrrci 1024, 0 : () -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrwi_rw = riscv.csrrwi 1024, 1 : () -> (!riscv.reg<x$>)
  // CHECK-NEXT: %csrrwi_w = riscv.csrrwi 1024, w, 1 : () -> (!riscv.reg<x$>)
  ...
  // CHECK-NEXT: %mul = riscv.mul %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %mulh = riscv.mulh %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %mulhsu = riscv.mulhsu %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %mulhu = riscv.mulhu %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  ...
  // CHECK-NEXT: %div = riscv.div %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %divu = riscv.divu %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %rem = riscv.rem %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %remu = riscv.remu %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>)

  // Assembler pseudo-instructions

  %li = "riscv.li"() {"immediate" = 1 : i32}: () -> !riscv.reg<x$>
  // CHECK-NEXT: %li = riscv.li 1 : () -> (!riscv.reg<x$>)
  ...
  // CHECK-NEXT:   %nested_li = riscv.li 1 : () -> (!riscv.reg<x$>)
  // CHECK-NEXT: }) : () -> ()
  ...
  // CHECK-NEXT: %custom0, %custom1 = riscv.custom_assembly_instruction "hello" %0, %1 : (!riscv.reg<x$>, !riscv.reg<x$>) -> (!riscv.reg<x$>, !riscv.reg<x$>)
  ...
  // CHECK-NEXT: %f0 = riscv.get_float_register : () -> (!riscv.freg<f$>)
  // CHECK-NEXT: %f1 = riscv.get_float_register : () -> (!riscv.freg<f$>)
  // CHECK-NEXT: %f2 = riscv.get_float_register : () -> (!riscv.freg<f$>)
  ...
  // CHECK-NEXT: %fmadd_s = riscv.fmadd.s %f0, %f1, %f2 : (!riscv.freg<f$>, !riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fmsub_s = riscv.fmsub.s %f0, %f1, %f2 : (!riscv.freg<f$>, !riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fnmsub_s = riscv.fnmsub.s %f0, %f1, %f2 : (!riscv.freg<f$>, !riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fnmadd_s = riscv.fnmadd.s %f0, %f1, %f2 : (!riscv.freg<f$>, !riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  ...
  // CHECK-NEXT: %fadd_s = riscv.fadd.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fsub_s = riscv.fsub.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fmul_s = riscv.fmul.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fdiv_s = riscv.fdiv.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fsqrt_s = riscv.fsqrt.s %f0 : (!riscv.freg<f$>) -> (!riscv.freg<f$>)
  ...
  // CHECK-NEXT: %fsgnj_s = riscv.fsgnj.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fsgnjn_s = riscv.fsgnjn.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fsgnjx_s = riscv.fsgnjx.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  ...
  // CHECK-NEXT: %fmin_s = riscv.fmin.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fmax_s = riscv.fmax.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.freg<f$>)
  ...
  // CHECK-NEXT: %fcvt_w_s = riscv.fcvt.w.s %f0 : (!riscv.freg<f$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %fcvt_wu_s = riscv.fcvt.wu.s %f0 : (!riscv.freg<f$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %fmv_x_w = riscv.fmv.x.w %f0 : (!riscv.freg<f$>) -> (!riscv.reg<x$>)
  ...
  // CHECK-NEXT: %feq_s = riscv.feq.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %flt_s = riscv.flt.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %fle_s = riscv.fle.s %f0, %f1 : (!riscv.freg<f$>, !riscv.freg<f$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %fclass_s = riscv.fclass.s %f0 : (!riscv.freg<f$>) -> (!riscv.reg<x$>)
  // CHECK-NEXT: %fcvt_s_w = riscv.fcvt.s.w %0 : (!riscv.reg<x$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fcvt_s_wu = riscv.fcvt.s.wu %0 : (!riscv.reg<x$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: %fmv_w_x = riscv.fmv.w.x %0 : (!riscv.reg<x$>) -> (!riscv.freg<f$>)

  %flw = "riscv.flw"(%0) {"immediate" = 1 : i32}: (!riscv.reg<x$>) -> !riscv.freg<f$>
  "riscv.fsw"(%0, %f0) {"immediate" = 1 : i32} : (!riscv.reg<x$>, !riscv.freg<f$>) -> ()
  // CHECK-NEXT: %flw = riscv.flw %0, 1 : (!riscv.reg<x$>) -> (!riscv.freg<f$>)
  // CHECK-NEXT: riscv.fsw %0, %f0, 1 : (!riscv.reg<x$>, !riscv.freg<f$>) -> ()
