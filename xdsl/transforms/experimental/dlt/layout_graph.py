...
class LayoutGraph:
    def __init__(
        self,
        identifier_uses: dict[StringAttr, set[SSAValue]] = None,
    ...
    def add_extent_constraint(
        self, ident: StringAttr, extent: dlt.InitDefinedExtentAttr
    ):
    ...
    def get_entry_layouts(self) -> dict[StringAttr, dlt.Layout]:
        return {
            ...
            if k not in [e.end for e in self.edges]
        }
    ...
    def propagate_type(
        self, ident: StringAttr, current_types: dict[StringAttr, dlt.PtrType]
    ):
        ...
        for edge in [e for e in self.edges if e.start == ident]:
            ...
            new_end = self.propagate_ptr(
                start_pointer, edge, end_pointer, current_types
            )
    ...
    def backpropagate_type(
        ...
        for edge in [e for e in self.edges if e.end == ident]:
            ...
            new_start = self.backpropagate_ptr(
                end_pointer, edge, start_pointer, current_types
            )
    ...
    def propagate_ptr(
        ...
        current_types: dict[StringAttr, dlt.PtrType],
    ) -> dlt.PtrType:
    ...
    def get_transitive_extent_constraints(
        self, ident: StringAttr, seen: set[StringAttr] = None
