...
%a_stream = "snitch_stream.strided_read"(%A, %sp2) {"dm" = #builtin.int<0>, "rank" = #builtin.int<2>} : (!riscv.reg<>, !snitch_stream.stride_pattern_type<2>) -> !stream.readable<!riscv.freg<ft0>>
...
%b_stream = "snitch_stream.strided_read"(%B, %sp2) {"dm" = #builtin.int<1>, "rank" = #builtin.int<2>} : (!riscv.reg<>, !snitch_stream.stride_pattern_type<2>) -> !stream.readable<!riscv.freg<ft1>>
...
%c_stream = "snitch_stream.strided_write"(%C, %sp2) {"dm" = #builtin.int<2>, "rank" = #builtin.int<2>} : (!riscv.reg<>, !snitch_stream.stride_pattern_type<2>) -> !stream.writable<!riscv.freg<ft2>>
...
"snitch_stream.generic"(%4, %a_stream, %b_stream, %c_stream) <{"operandSegmentSizes" = array<i32: 1, 2, 1>}> ({
^0(%a : !riscv.freg<ft0>, %b : !riscv.freg<ft1>):
%c = riscv.fadd.d %a, %b : (!riscv.freg<ft0>, !riscv.freg<ft1>) -> !riscv.freg<ft2>
snitch_stream.yield %c : !riscv.freg<ft2>
