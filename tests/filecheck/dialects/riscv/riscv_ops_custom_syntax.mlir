...
"builtin.module"() ({
  ...
  // CHECK-NEXT: %addi = riscv.addi %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %slti = riscv.slti %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %sltiu = riscv.sltiu %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %andi = riscv.andi %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %ori = riscv.ori %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %xori = riscv.xori %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %slli = riscv.slli %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %srli = riscv.srli %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %srai = riscv.srai %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %lui = riscv.lui 1 : !riscv.reg<>
  // CHECK-NEXT: %auipc = riscv.auipc 1 : !riscv.reg<>
  // CHECK-NEXT: %mv = riscv.mv %0 : !riscv.reg<>
  ...
  "riscv.nop"() : () -> ()
  // CHECK-NEXT: %add = riscv.add %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %slt = riscv.slt %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %sltu = riscv.sltu %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %and = riscv.and %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %or = riscv.or %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %xor = riscv.xor %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %sll = riscv.sll %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %srl = riscv.srl %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %sub = riscv.sub %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %sra = riscv.sra %0, %1 : !riscv.reg<>
  ...
  // CHECK-NEXT: riscv.jalr %0, 1 :
  // CHECK-NEXT: riscv.jalr %0, 1, None :
  // CHECK-NEXT: riscv.jalr %0, "label" :
  ...
  // CHECK-NEXT: riscv.beq %0, %1, 1 :
  // CHECK-NEXT: riscv.bne %0, %1, 1 :
  // CHECK-NEXT: riscv.blt %0, %1, 1 :
  // CHECK-NEXT: riscv.bge %0, %1, 1 :
  // CHECK-NEXT: riscv.bltu %0, %1, 1 :
  // CHECK-NEXT: riscv.bgeu %0, %1, 1 :
  ...
  // CHECK-NEXT: %lb = riscv.lb %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %lbu = riscv.lbu %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %lh = riscv.lh %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %lhu = riscv.lhu %0, 1 : !riscv.reg<>
  // CHECK-NEXT: %lw = riscv.lw %0, 1 : !riscv.reg<>
  // CHECK-NEXT: riscv.sb %0, %1, 1 :
  // CHECK-NEXT: riscv.sh %0, %1, 1 :
  // CHECK-NEXT: riscv.sw %0, %1, 1 :
  ...
  // CHECK-NEXT: %csrrw_rw = riscv.csrrw %0, 1024 : !riscv.reg<>
  // CHECK-NEXT: %csrrw_w = riscv.csrrw %0, 1024, w : !riscv.reg<>
  // CHECK-NEXT: %csrrs_rw = riscv.csrrs %0, 1024 : !riscv.reg<>
  // CHECK-NEXT: %csrrs_r = riscv.csrrs %0, 1024, r : !riscv.reg<>
  // CHECK-NEXT: %csrrc_rw = riscv.csrrc %0, 1024 : !riscv.reg<>
  // CHECK-NEXT: %csrrc_r = riscv.csrrc %0, 1024, r : !riscv.reg<>
  ...
  // CHECK-NEXT: %mul = riscv.mul %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %mulh = riscv.mulh %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %mulhsu = riscv.mulhsu %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %mulhu = riscv.mulhu %0, %1 : !riscv.reg<>
  ...
  // CHECK-NEXT: %div = riscv.div %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %divu = riscv.divu %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %rem = riscv.rem %0, %1 : !riscv.reg<>
  // CHECK-NEXT: %remu = riscv.remu %0, %1 : !riscv.reg<>
  ...
  // CHECK-NEXT: %custom0, %custom1 = riscv.custom_assembly_instruction %0, %1, "hello" : !riscv.reg<>, !riscv.reg<>


  // RISC-V extensions
  "riscv.scfgw"(%0, %1) : (!riscv.reg<>, !riscv.reg<>) -> ()
  // CHECK-NEXT: riscv.scfgw %0, %1 :
  ...
  // CHECK-NEXT: %fmadd_s = riscv.fmadd.s %f0, %f1, %f2 : !riscv.freg<>
  // CHECK-NEXT: %fmsub_s = riscv.fmsub.s %f0, %f1, %f2 : !riscv.freg<>
  // CHECK-NEXT: %fnmsub_s = riscv.fnmsub.s %f0, %f1, %f2 : !riscv.freg<>
  // CHECK-NEXT: %fnmadd_s = riscv.fnmadd.s %f0, %f1, %f2 : !riscv.freg<>
  ...
  // CHECK-NEXT: %fadd_s = riscv.fadd.s %f0, %f1 : !riscv.freg<>
  // CHECK-NEXT: %fsub_s = riscv.fsub.s %f0, %f1 : !riscv.freg<>
  // CHECK-NEXT: %fmul_s = riscv.fmul.s %f0, %f1 : !riscv.freg<>
  // CHECK-NEXT: %fdiv_s = riscv.fdiv.s %f0, %f1 : !riscv.freg<>
  // CHECK-NEXT: %fsqrt_s = riscv.fsqrt.s %f0 : !riscv.freg<>
  ...
  // CHECK-NEXT: %fsgnj_s = riscv.fsgnj.s %f0, %f1 : !riscv.freg<>
  // CHECK-NEXT: %fsgnjn_s = riscv.fsgnjn.s %f0, %f1 : !riscv.freg<>
  // CHECK-NEXT: %fsgnjx_s = riscv.fsgnjx.s %f0, %f1 : !riscv.freg<>

  %fmin_s = "riscv.fmin.s"(%f0, %f1) : (!riscv.freg<>, !riscv.freg<>) -> !riscv.freg<>
  %fmax_s = "riscv.fmax.s"(%f0, %f1) : (!riscv.freg<>, !riscv.freg<>) -> !riscv.freg<>
  // CHECK-NEXT: %fmin_s = riscv.fmin.s %f0, %f1 : !riscv.freg<>
  // CHECK-NEXT: %fmax_s = riscv.fmax.s %f0, %f1 : !riscv.freg<>
  ...
  // CHECK-NEXT: %fcvt_w_s = riscv.fcvt.w.s %f0 : !riscv.reg<>
  // CHECK-NEXT: %fcvt_wu_s = riscv.fcvt.wu.s %f0 : !riscv.reg<>
  // CHECK-NEXT: %fmv_x_w = riscv.fmv.x.w %f0 : !riscv.reg<>
  ...
  // CHECK-NEXT: %feq_s = riscv.feq.s %f0, %f1 : !riscv.reg<>
  // CHECK-NEXT: %flt_s = riscv.flt.s %f0, %f1 : !riscv.reg<>
  // CHECK-NEXT: %fle_s = riscv.fle.s %f0, %f1 : !riscv.reg<>
  // CHECK-NEXT: %fclass_s = riscv.fclass.s %f0 : !riscv.reg<>
  // CHECK-NEXT: %fcvt_s_w = riscv.fcvt.s.w %0 : !riscv.freg<>
  // CHECK-NEXT: %fcvt_s_wu = riscv.fcvt.s.wu %0 : !riscv.freg<>
  // CHECK-NEXT: %fmv_w_x = riscv.fmv.w.x %0 : !riscv.freg<>

  %flw = "riscv.flw"(%0) {"immediate" = 1 : i32}: (!riscv.reg<>) -> !riscv.freg<>
  "riscv.fsw"(%0, %f0) {"immediate" = 1 : i32} : (!riscv.reg<>, !riscv.freg<>) -> ()
  // CHECK-NEXT: %flw = riscv.flw %0 {"immediate" = 1 : i32} : !riscv.freg<>
  // CHECK-NEXT: riscv.fsw %0, %f0 {"immediate" = 1 : i32} :
