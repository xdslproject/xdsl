...
EmitCIntegerTypeBitwidthConstr = functools.reduce(
    operator.or_, (EqAttrConstraint(bw) for bw in (i1, i8, i16, i32, i64))
)

EmitCIntegerTypeConstr = ParamAttrConstraint(
    IntegerAttr, (EmitCIntegerTypeBitwidthConstr, AnyAttr())
)
"""
Supported integer types in EmitC.
...
EmitCPointerWideTypesConstr = AnyOf(
    [EmitC_PtrDiffT(), EmitC_SignedSizeT(), EmitC_SizeT()]
)

EmitCArrayElementTypeConstr = AnyOf(
    [
        EmitCIntegerTypeConstr,
        IndexType(),
        EmitC_OpaqueType,
        EmitCPointerWideTypesConstr,
        EmitCFloatTypeConstr,
    ]
)
"""
Supported element types in EmitC arrays.
...
class EmitC_ArrayType(
    ...
    element_type: EmitCArrayElementTypeConstr
...
class EmitC_PointerType(ParametrizedAttribute, TypeAttribute):
    ...
    name = "emitc.ptr"
    pointee_type: EmitC_LValueType


EmitCTypeConstr = AnyOf(
    [
        EmitCIntegerTypeConstr,
        EmitCFloatTypeConstr,
        IndexType(),
