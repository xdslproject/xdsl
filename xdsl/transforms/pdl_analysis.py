...
class AnalyzedPDLOperation:
    ...
    def is_terminator(self):
        return self.pdl_op.has_trait(IsTerminator)
...
class PDLAnalysis:
    ...
    def __init__(self, context: MLContext, pattern_op: pdl.PatternOp):
        ...
        for lhs_op in pattern_op.body.block.ops_reverse:
    ...
    def dominance_analysis(self):
        ...
        if len(self.rewrite_op.body.blocks) != 1:
            return
    ...
    def _analyze_pdl_rewrite_op(self, pdl_rewrite_op: pdl.RewriteOp):
        if pdl_rewrite_op.body and len(pdl_rewrite_op.body.blocks) == 1:
...
class PDLAnalysisPass(ModulePass):
    """
    Analyses PDL patterns
    """
    ...
    def apply(self, ctx: MLContext, op: ModuleOp) -> None:
        for nested_op in op.ops:
            if isinstance(nested_op, pdl.PatternOp):
                debug(f"Found pattern:{nested_op.name}")
                analysis = PDLAnalysis(ctx, nested_op)
                analysis.terminator_analysis()
                analysis.dominance_analysis()
