...
builtin.module {
  ...
  func.func private @memref_test() {
    ...
    %17 = memref.alloca(%12) {alignment = 0 : i64} : memref<?xindex>
    %18 = memref.alloca(%12, %13, %14) {alignment = 0 : i64} : memref<?x?x?xindex>
...
// CHECK-NEXT:     %{{.*}} = memref.alloca(%{{.*}}) {alignment = 0 : i64} : memref<?xindex>
// CHECK-NEXT:     %{{.*}} = memref.alloca(%{{.*}}, %{{.*}}, %{{.*}}) {alignment = 0 : i64} : memref<?x?x?xindex>
